...
public class CommentController {
    ...
    @PutMapping("board/{boardId}/article/{articleId}/comment/{commentId}")
    public ResponseEntity<Response<Long>> editComment(@PathVariable Long boardId,
        @PathVariable Long articleId,
        @PathVariable Long commentId, Authentication authentication,
        @RequestBody UpdateCommentRequest request) {
        return ResponseEntity.ok(Response.success(
            commentService.editComment(boardId, articleId, commentId, authentication.getName(),
                request)
        ));
    }


    @DeleteMapping("board/{boardId}/article/{articleId}/comment/{commentId}")
    public ResponseEntity<Response<Void>> deleteArticle(@PathVariable Long boardId,
        @PathVariable Long articleId, @PathVariable Long commentId, Authentication authentication) {
        commentService.deleteComment(boardId, articleId, commentId, authentication.getName());
