...
public interface ArticleEntityRepository extends JpaRepository<ArticleEntity, Long> {
    ...
    @Query("select a from ArticleEntity a where a.board.id = :boardId order by a.createdAt desc")
    List<ArticleEntity> findAllByBoardIdLOrderByCreatedDateDesc(Long boardId);

    @Query("select a from ArticleEntity a where a.board.id = :boardId and a.id < :articleId order by a.createdAt desc")
